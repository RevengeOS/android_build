...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the RevengeOS-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/revengeos/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/revengeos/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
